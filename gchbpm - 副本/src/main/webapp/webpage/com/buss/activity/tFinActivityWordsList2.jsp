...
<%--    <t:dgCol title="创建日期"  field="createDate" hidden="true"  formatter="yyyy-MM-dd"  queryMode="single"  width="120"></t:dgCol> --%>
   ...
   <t:dgToolBar title="录入" icon="icon-add" url="tFinActivityWordsController.do?goAdd&finActId=${finActId }&wordsType=2" funname="add" height="400"></t:dgToolBar>
   <t:dgToolBar title="选择" icon="icon-add" url="templateWordsGoodsController.do?goodsWordsForGoods&finActId=${finActId }" funname="chooseWords" ></t:dgToolBar>
 		...
 		$.dialog({
		    ...
		    ok: function(){
		    	...
		    	if (selected == '' || selected == null ){
					...
					var url = "tFinActivityWordsController.do?doBatchAddGoodsWords";
				$.ajax({
					url : url,
					type : 'post',
					data : {
