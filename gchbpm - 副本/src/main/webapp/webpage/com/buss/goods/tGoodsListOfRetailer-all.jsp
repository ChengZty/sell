...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tGoodsList" checkbox="false" fitColumns="true"  title="商品列表" sortName="goodsUpdateTime" sortOrder="desc" actionUrl="tGoodsController.do?datagrid&goods_status=${goods_status }"
   ...
   <t:dgCol title="商品状态"  field="goodsStatus" align="center" dictionary="g_status" query="true"  queryMode="single" width="120" ></t:dgCol>
   <t:dgCol title="操作" field="opt" width="50"></t:dgCol>
    <t:dgFunOpt funname="goUpdateRecommondPics(id)"  title="修改推荐"></t:dgFunOpt>
   <t:dgToolBar title="查看" icon="icon-search" url="" onclick="goView()"  funname="detail"></t:dgToolBar>
   <t:dgToolBar title="导入" icon="icon-put" funname="ImportXls"></t:dgToolBar>
<%--    <t:dgToolBar title="导出" icon="icon-putout" funname="ExportXls"></t:dgToolBar> --%>
   <t:dgToolBar title="模板下载" icon="icon-putout" funname="downloadTemp"></t:dgToolBar>
  </t:datagrid>
  </div>
 </div>
...
//  			$("#tGoodsListtb").find("input[name='updateDate']").attr("class","Wdate").attr("style","height:20px;width:90px;").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 ...
 function goUpdateRecommondPics(id) {
	 var url = "tGoodsController.do?goUpdateRecommondPics&id="+id;
...
//导入
 function ImportXls() {
 	openuploadwin('Excel导入', 'tGoodsController.do?upload&newGoodsType=1', "tGoodsList");
 }

 //模板下载
 function downloadTemp() {
 	JeecgExcelExport("tGoodsController.do?downloadTemp","tGoodsList");
 }
 </script>