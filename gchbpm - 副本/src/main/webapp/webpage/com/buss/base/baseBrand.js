...
//                 var key = directory+"baseBrand/"+time+"/"+name;
//                 return key
//             }
//         }
//     });
...
//     });
    ...
    new BtnSelectFileVo('#pickfiles_1', paramsJson_1);  //按钮点击选择
    new DragDropSelectFileVo('#container_div_1', paramsJson_1);  //拖曳选择

    //参数
    var paramsJson_2 = {
        cropperWidth: 720, //裁剪宽度
        cropperHeight: 250, //裁剪高度
        moduleName: 'baseBrand',  //模块名称
        dealSourceLinkFn: function(sourceLink){  //处理上传图片路径
            var div_imgs = "<div class='pic_div'><a class='delete' onclick='delPic(this)' href='#'>×</a><img src='"+sourceLink+"'  height='100' ></img></div>"
            $("#container_div_2").html(div_imgs);
    ...
    new BtnSelectFileVo('#pickfiles_2', paramsJson_2);  //按钮点击选择
	new DragDropSelectFileVo('#container_div_2', paramsJson_2);  //拖曳选择
})
...
function back() {
  document.location="baseBrandController.do?list";
}
...
var leipiEditor = UE.getEditor('content_ue',{
    toolleipi:true,//是否显示，设计器的 toolbars
    textarea: 'brandDesc.content',   
    toolbars: [[
...
function setContent(){
    ...
    if(leipiEditor.hasContents()){
        leipiEditor.sync();
	    $("#content").val(leipiEditor.getContent());
	}
    var flag = true;
	return flag;
}
