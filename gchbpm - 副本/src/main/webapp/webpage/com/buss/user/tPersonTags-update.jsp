...
//获取职称
  ...
  //回显名称
  function initName(){
	  $("#topCategoryId").val("${tPersonTagsPage.topCategoryId }");
	  $("#professionalCode").val("${tPersonTagsPage.professionalCode }");
  }
 ...
 <body>
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="tPersonTagsController.do?doUpdate" tiptype="4" beforeSubmit="setName()">
					...
					<input id="status" name="status" type="hidden" value="${tPersonTagsPage.status }">
					<input id="userId" name="userId" type="hidden" value="${tPersonTagsPage.userId }">
					<input id="categries" name="categries" type="hidden" value=""><!-- 类目 -->
					<input id="topCategoryName" name="topCategoryName" type="hidden" value="${tPersonTagsPage.professionalName }"><!-- 一级类目名称 -->
					<input id="professionalName" name="professionalName" type="hidden" value="${tPersonTagsPage.professionalName }"><!-- 职称名称 -->
		<table  cellpadding="0" cellspacing="1" class="formtable">
				<tr>
					...
					<tr>
						...
						<td class="value">
									<t:dictSelect field="tagCode" type="list" 
										typeGroupCode="hp_type" defaultVal="${tPersonTagsPage.tagCode}" hasLabel="false" id="tag" title="标签"></t:dictSelect>     
										<a href="#" class="easyui-linkbutton" plain="true" style="margin-left: 50px" onclick="setVisibleCatgs('${tPersonTagsPage.id }')" ><span class="icon-search l-btn-icon-left">选择类目</span></a>
							<span class="Validform_checktip"></span>
					...
					<td class="value">
							<select name="professionalCode" id="professionalCode" datatype="*">
								<c:forEach var="pro" items="${professionList}">
									<option value="${pro.professionalCode }">${pro.professionalName}</option>
									</c:forEach>
							</select>
							<span class="Validform_checktip"></span>
					...
					<tr id="c_pic"  <c:if test="${empty tPersonTagsPage.picUrl }"> style="display: none"</c:if> >
					<td align="right">
						<label class="Validform_label">
							证书:
						</label>
					...
					<td>
	        			...
	        			<div  id="main_pics" style="height: 130px;">
	        				<c:forTokens items="${tPersonTagsPage.picUrl }" delims="," var="url">
	        				<div style='float:left'><a class='delete' onclick='delPic(this)' href='#'></a><img src='${url }' width='100' height='100' ></img></div>
	        				</c:forTokens>
	        			</div>
<%-- 					    <img id="prePic"     src="${tPersonTagsPage.picUrl }"   width="200px" height="150px"    /> --%>
					    <input id="picUrl" name="picUrl" type="hidden" value="${tPersonTagsPage.picUrl }" errormsg="请上传证书图片" >
					    <span class="Validform_checktip"></span>
