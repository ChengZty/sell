...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tNoticeCountList" checkbox="true" fitColumns="false" title="公司通知导购浏览统计报表" actionUrl="guideNoticeCountController.do?tNoticeCountDatagrid" idField="id" fit="true" queryMode="group">
   <t:dgCol title="id"  field="id"  hidden="true"  queryMode="group"  width="120"></t:dgCol>
   <t:dgCol title="查询时间"  field="searchTime"  hidden="true" query="true"   queryMode="group"  width="150"></t:dgCol>
   <t:dgCol title="公司通知标题"  field="title"  query="true"    width="200"></t:dgCol>
 ...
 $(document).ready(function(){
	//给时间控件加上样式
		$("#tNoticeCountListtb").find("input[name='searchTime_begin']").attr("class","Wdate").attr("style","height:20px;width:90px;").val("${searchTime_begin}")
				.attr("id","searchTime_begin").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'searchTime_end\')}'});});
		$("#tNoticeCountListtb").find("input[name='searchTime_end']").attr("class","Wdate").attr("style","height:20px;width:90px;").val("${searchTime_end}")
...
//导出
function ExportXls() {
	JeecgExcelExport("guideNoticeCountController.do?exportNoticeXls","tNoticeCountList");
}
