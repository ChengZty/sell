...
<html>
 ...
 <body>
  <t:formvalid formid="formobj" dialog="false" usePlugin="password" layout="table" action="tSmsSendInfoController.do?doAdd" callback="backList" tiptype="4">
					...
					<input id="sendStatus" name="sendStatus" type="hidden" value="0">
					<input id="status" name="status" type="hidden" value="A">
					<input id="msgNum" name="msgNum" type="hidden" value="1">
					<input id="source" name="source" type="hidden" value="${source }">
					<input id="sourcePushType" name="sourcePushType" type="hidden" value="${sourcePushType }">
		...
		<table style="width: 800px;height: 100%;" cellpadding="0" cellspacing="1" class="formtable">
				...
				<tr>
					...
					<td class="value">
					     	<input id="title" name="title" type="text" style="width: 300px" class="inputxt" value="${title }">
				...
				<tr>
					...
					<td class="value">
					     	<input id="pushUrl" name="pushUrl" type="text" style="width: 300px" ondblclick="getDateSource(this)" class="inputxt" value="${pushUrl }" placeholder="双击显示可选列表"> 
				...
				<tr>
					...
					<td class="value">
							<div style="position: relative">
						     	 <textarea class="inputxt"  id="content" name="content" style="width: 350px;height: 130px;" onkeyup="setPreviewHtml()"></textarea>
				...
				<!--  <tr>
					...
					<td class="value">
					     	 <input id="contentEnd" name="contentEnd" type="text" style="width: 300px" class="inputxt" onkeyup="setPreviewHtml()">
							<span class="Validform_checktip"></span>
...
<!-- 							<a href="#" class="easyui-linkbutton l-btn" onclick="goCustomerList()">选择顾客</a>  -->
							...
							<a href="#" class="easyui-linkbutton l-btn l-btn-plain" plain="true"  onclick="goCustomerList()" id="goCustomerList"><span class="icon-search l-btn-icon-left">选择顾客</span></a>已选中（<span id="selectCount" style="color:red;">0</span>）人
							<span class="Validform_checktip"></span>
...
<!-- 						<a href="#" class="easyui-linkbutton l-btn" id="btn_view" onclick="view()" iconCls="icon-msg">预览</a>  -->
  ...
  <script type="text/javascript">
	...
	var sourcePushType = $("#sourcePushType").val();//来源推送类型,0海报，1商品
  	$(function(){
	   var source = $("#source").val();//来源，从哪里跳转进来，0代表从列表中的录入或者编辑按钮进入，其他代表其他地方进入
		//判断是否从商品列表或者资讯列表的推送按钮进入，如果是商品或者资讯的推送按钮进入则居中显示，并把返回按钮删除
		if("0" == sourcePushType){
			$("#pushType0").attr("checked","checked");
		}else if("1" == sourcePushType){
			$("#pushType1").attr("checked","checked");
		}
		if("0"==source){
			$("#alignLeftOrCenter").attr("align","left");
		}else{
			$("#alignLeftOrCenter").attr("align","center");
			$("#btn_reset").remove();
		}
