...
//                    var div_imgs = "<div class='pic_div'><a class='delete' onclick='delPic(this)' href='#'>×</a><img src='"+sourceLink+"' width='180' height='320' ></img>"
//      				+"<div><input type='hidden' class='picUrl' value='"+sourceLink+"'/></div></div>";
...
//                 var key = directory+"storePics/"+time+"/"+name;
//                 return key
//             }
//         }
//     });
    ...
    //七牛上传
    var paramsJson = {
        cropperWidth: 720, //裁剪宽度
        cropperHeight: 1280, //裁剪高度
        moduleName: 'storePics',  //模块名称
    ...
    new BtnSelectFileVo('#pickfiles_1', paramsJson);  //按钮点击选择
    new DragDropSelectFileVo('#container_div_1', paramsJson);  //拖曳选择
...
function setPicContent(){
	var flag = true;
	var pic_divs = $("#container_div_1").find("div.pic_div");
	if($(pic_divs).length>0){
		var n=0;
		$(pic_divs).each(function(){
			$(this).find("input.id").attr("name","storePics["+n+"].id");
			$(this).find("input.picUrl").attr("name","storePics["+n+"].picUrl");
			n++;
		 });
	}else{
		flag = false;
		$.messager.alert('错误提示', '请上传店铺图片');
	}
	return flag;
}

//检查店铺编号是否存在
$('#storeCode').on('blur', function(){
	checkCode();
});
...
function checkCode(){
	$.ajax({
	       cache: true,
	       type: "POST",
	       url: "tStoreController.do?checkStoreCode",
	       data: "storeCode=" + $("#storeCode").val()+"&id=" + $("#id").val(),
	       datatype: 'json',
	       async: false,
