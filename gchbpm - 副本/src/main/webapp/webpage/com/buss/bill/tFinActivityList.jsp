...
<div id="main_depart_list" class="easyui-layout" fit="true">
  ...
  <t:datagrid name="tFinActivityList" checkbox="false" fitColumns="false" title="导购活动" actionUrl="tFinActivityController.do?datagrid" 
  idField="id" fit="true" queryMode="group" sortName="createDate" sortOrder="desc">
   ...
   <t:dgCol title="零售商"  field="retailerName"   queryMode="single"  width="120"></t:dgCol>
   ...
   <t:dgCol title="活动状态"  field="activityStatus" replace="待审核_1,已审核_2,已下架_3" query="true" queryMode="single"  width="90"></t:dgCol>
...
<%--    <t:dgFunOpt funname="updatebytab(id)" title="编辑" exp="activityStatus#eq#1&&platformType#eq#1"></t:dgFunOpt> --%>
   <t:dgConfOpt url="tFinActivityController.do?doDel&id={id}"  message="确定删除" title="删除" exp="activityStatus#eq#1"></t:dgConfOpt>
   <t:dgFunOpt funname="getFinActivityGoodsList(id,activityStatus,activityName,platformType,retailerId)" title="活动商品" exp="activityType#eq#1"></t:dgFunOpt>
...
<div class="easyui-panel" style="padding: 1px;" fit="true" border="false" id="function-panel"></div>
 ...
 function addbytab() {
		document.location="tFinActivityController.do?goAdd&platformType=1";
}
...
function updatebytab(id){
 ...
 function getFinActivityGoodsList(id,activityStatus,activityName,platformType,retailerId) {
	 if(li_east == 0){
	...
	$('#function-panel').panel("refresh","tFinActivityGoodsController.do?list&fin_act_Id="+id+"&aSts="+activityStatus+"&pType="+platformType+"&rId="+retailerId);
	}
 //话术，审核也可以修改
...
//重置
 function searchMainReset(name) {
 		...
 		$('#'+name).datagrid(
 			{
 				url : 'tFinActivityController.do?datagrid&field=id,updateDate,platformType,activityName,activityType,brandName,startTime,startTime_begin,startTime_end,endTime,endTime_begin,endTime_end,retailerId,retailerName,auditTime,auditor,activityStatus,toUserType,status,',
 				pageNumber : 1
 			});
 }
 </script>