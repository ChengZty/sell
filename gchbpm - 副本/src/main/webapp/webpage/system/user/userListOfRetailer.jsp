...
<script>
    $(function() {
    		$("select[name='userType']").find("option[value='01']").remove();//移除‘后台’用户类型
    		$("select[name='userType']").find("option[value='02']").remove();//移除‘零售商’用户类型
//    		$("select[name='userType']").val("03");
...
<t:datagrid name="userList" title="操作" actionUrl="userController.do?datagrid" 
	...
	<c:if test="${userType =='01'}"><t:dgCol title="手机号" field="mobilePhone" ></t:dgCol></c:if>
<%-- 	<t:dgCol title="角色" field="userKey" ></t:dgCol> --%>
	...
	<t:dgFunOpt funname="revokeCustInfo(id,realName)"   title="回收顾客资料"   exp="userType#eq#03&&userStatus#eq#0"></t:dgFunOpt>
	<t:dgToolBar title="录入" langArg="common.user" icon="icon-add" url="userController.do?goAdd" funname="add"></t:dgToolBar>
	<t:dgToolBar title="编辑" langArg="common.user" icon="icon-edit" url="userController.do?goUpdate" funname="update"></t:dgToolBar>
	<t:dgToolBar title="密码重置" icon="icon-edit" url="userController.do?changepasswordforuser" funname="update" width="400" height="200"></t:dgToolBar>
	<t:dgToolBar title="锁定" icon="icon-edit" url="userController.do?lock&lockvalue=2" funname="lockObj"></t:dgToolBar>
	<t:dgToolBar title="激活" icon="icon-edit" url="userController.do?lock&lockvalue=1" funname="unlockObj"></t:dgToolBar>
	<t:dgToolBar title="停用" icon="icon-edit" url="userController.do?lock&lockvalue=0" funname="forbidObj"></t:dgToolBar>
	<t:dgToolBar title="刷新所属区域" icon="icon-reload" url="userController.do?updateArea" funname="updateArea"></t:dgToolBar>
...
//刷新所属区域
function updateArea(title,url, gname) {
	gridname=gname;
	$.dialog.confirm('确定刷新所属区域？', function(){
		lockuploadify(url);
	}, function(){
	});
}
 ...
 */
function revokeCustInfo(id,realName)
{
	$.dialog.confirm('确定要回收'+realName+'的顾客资料吗？', function(){
		$.ajax({
			async : false,
			cache : false,
			type : 'POST',
			url : 'tFocusCustomerController.do?revokeCustInfo&guideId='+id,// 请求的action路径
			error : function() {// 请求失败处理函数
			},
			success : function(data) {
				var d = $.parseJSON(data);
