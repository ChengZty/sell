(function($, win, undef) {
	...
	Validform.util = {
		...
		showmsg : function(msg, type, o, triggered) {
			...
			if (type == 1 && o.type != 2 || triggered == "byajax" && type != 4 && type !=5) {
				msghidden = false;
				msgobj.find(".iframe").css("height", msgobj.outerHeight());
				msgobj.show();
				setCenter(msgobj, 100);
			}
			if (msg == "通过信息验证！" && type !=5) {
				setTimeout(function() {
					msgobj.hide();
				}, 1000);
			}
			...
			if ((type == 5) && o.obj) {
				...
				//'#78BA32'
				//alert(o.obj.attr("id"));
				// layer.tips(msg,o.obj.attr("id"), {tips: [o.obj.attr("path"), o.obj.attr("color")]});
				var obj = o.obj;
				var ptah;
				var color;
				var time;
				if(typeof(obj.attr("path"))=="undefined")
				{
					path = 2;	
				}else{
					path = obj.attr("path");
				...
				if(msg == "通过信息验证！"){
					color = "#78BA32";
				}else if(typeof(obj.attr("color"))=="undefined"){
					color = "red";
				}else{
					color = obj.attr("color");
				}
				if(typeof(obj.attr("time"))=="undefined"){
					time = 5000;
				}else{
					time = obj.attr("time");
				}
				//alert(path+":"+color+":"+time);
				layer.tips(msg,"#"+obj.attr("id"),{tips: [path,color],time: time});
			}
		},
