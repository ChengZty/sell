...
//                    mimeTypes: 'image/*'
            ...
            function updateStatus() {
                ...
                if (state === 'ready') {
                    text = lang.updateStatusReady.replace('_', fileCount).replace('_KB', WebUploader.formatSize(fileSize)).replace('_maxNum',imageMaxNum);
            ...
            uploader.on('uploadBeforeSend', function (file, data, header) {
        		...
        		data["key"]=code+"/uploadimage/"+time+"/"+name;
            });
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS'||json.key) {
                    	...
                    	// update by huangbo  20160603  多图上传的时候，图片上传后，插入编辑器的顺序，不是多图上传界面的那个顺序，而是根据哪个先上传到服务器，就哪个在前。修改为按顺序排列
                        //_this.imageList.push(json);   
                        _this.imageList[$file.index()] = json;
                        $file.append('<span class="success"></span>');
                    } else {
                    	console.log(json);//七牛token问题
