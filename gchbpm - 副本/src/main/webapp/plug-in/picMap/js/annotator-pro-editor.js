...
//					self.end_moving();
		...
		start_moving : function(pageX, pageY) {
			...
			self.event_initial_x = pageX+15;
			self.event_initial_y = pageY+15;
...
//			self.obj_visible.removeClass('ndd-rect-style-4');
	...
	NDD_Annotation.prototype = {
		init : function() {
			...
			self.obj.append('<div class="ndd-annotation-box"><div class="del-pic-map" onclick="delPicMap(this)">×</div></div>');
			...
			self.obj_box.append('<a src=""><div class="ndd-annotation-content"></div></a>');
...
//			self.obj_content.append('<p onclick=findGoods(this)>'+ self.text +'</p>');
			self.obj_content.append('<div class="div_p" onclick=findGoods(this)>'+ self.text +'</div>');
		},
		show : function() {
			var self = this;
...
//			self.obj.removeClass('ndd-annotation-visible');
		...
		initialize_dimentions : function() {
			...
			if (self.drawable.settings.popup_position == "top") {
				self.left = -self.width/2 + self.drawable.width/2-15;
				self.top = -self.height - 15;
...
//				});
			...
			if (self.drawable.settings.popup_position == "bottom") {
				self.left = -self.width/2 + self.drawable.width/2-15;
				self.top = self.drawable.height + 20-35;
...
//				});
			...
			if (self.drawable.settings.popup_position == "left") {
				...
				self.top = -self.height/2 + self.drawable.height/2-15;
...
//				});
			...
			if (self.drawable.settings.popup_position == "right") {
				self.left = self.drawable.width + 20-35;
				self.top = -self.height/2 + self.drawable.height/2-15;
...
//			self.title = new_settings.title;
			var id = self.drawable.id;
			var content = $("#"+id).find("p").text();
...
//					self.obj_content.html('<p  onclick=findGoods(this)>'+ new_settings.text +'</p>');
					self.obj_content.html('<div class="div_p" onclick=findGoods(this)>'+ new_settings.text +'</div>');
				}
			} else {
...
//			});
	...
	function refresh_form() {
		...
		if ($('#input-id').val().length > 0) {
			$('#input-deep-link-url').html($('#input-id').val() + '/');
			$('#input-deep-link-url-help').html('Example: <code>' + escapeHTML('<a href="' + $('#input-id').val() + '"></a>') + '</code>');
		} else {
	...
	$(document).ready(function() {
		// return;
		init_canvas(div_width, div_height, function() {
			init_global_events();
			form_events();
			load_settings({});
			refresh_form();
