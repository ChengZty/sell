...
var windowapi = frameElement ? frameElement.api : null,W = windowapi==null?null:windowapi.opener;//内容页中调用窗口实例对象接口
 ...
 * 代入查询条件
 */
function JeecgExcelExportWithPars(url,datagridId,page,rows){
	var queryParams = $('#'+datagridId).datagrid('options').queryParams;
	$('#'+datagridId+'tb').find('*').each(function() {
	    queryParams[$(this).attr('name')] = $(this).val();
	});
 ...
 * @returns
 */
function isConfirm(flag,url,datagridId){
	$("#winPage").window('close');
	if(flag)
	{
		var page = $("#page").val();
		var rows = $("#rows").val();
		JeecgExcelExportWithPars(url,datagridId,page,rows);
	}
	return flag;
}
/**
 ...
 * @param datagridId
 */
function showWinPage(total,url,datagridId,pageNum){
	var temp = parseInt(total/pageNum);
	var mod = total%pageNum;
	if(mod > 0){
		temp = temp+1;
	}
	$("#winPage").empty();
	var pageHtml = "<form style='padding:5px 5px 5px 40px;'>"
	  			  +	"<p><span id='spanRow' style='color: red'></span></p>"
		          + "<p>当前页数: <input type='text' id='page' class='easyui-numberbox' value='1'  style='ime-mode:Disabled'></p>"
		          + "<p>固定条数: <input type='text' id='rows' class='easyui-numberbox' value='"+pageNum+"'  readonly='true'  style='ime-mode:Disabled;background-color: gray;'></p>"  
				  + "<div style='padding:5px;text-align:center;'>"  
				  +	            "<a href='#' class='easyui-linkbutton' icon='icon-save' onclick=\""+"isConfirm(true,'"+url+"','"+datagridId+"')\">确定</a>&nbsp;"  
				  +	            "<a href='#' class='easyui-linkbutton' icon='icon-cancel' onclick=\""+"isConfirm(false,'"+url+"','"+datagridId+"')\">取消</a>"  
				  + "</div></form>";
	$('#winPage').append(pageHtml);
	$.parser.parse($("#winPage"));
	$("#spanRow").html("总记录数："+total+"条,共"+temp+"页");
	$('#page').numberspinner({ 
			 required:true,  
		     increment:1,
		     min:1,
	...
	var $win = $('#winPage').window({title:'请选择导出页面数('+pageNum+'/页)'});
	$win.window('open');
}
