...
public class ChooseTag extends TagSupport {
	...
	public StringBuffer end() {
		String confirm = MutiLangUtil.getMutiLangInstance().getLang("确定");
		String cancel = MutiLangUtil.getMutiLangInstance().getLang("取消");
		String methodname = UUIDGenerator.generate().replaceAll("-", "");
		StringBuffer sb = new StringBuffer();
		sb.append("<a href=\"#\" class=\"easyui-linkbutton\" plain=\"true\" icon=\"" + icon + "\" onClick=\"choose_"+methodname+ StringUtil.replace("()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("选择", langArg)));
		if (isclear&&StringUtil.isNotEmpty(textname)) {
			sb.append("<a href=\"#\" class=\"easyui-linkbutton\" plain=\"true\" icon=\"icon-redo\" onClick=\"clearAll_"+methodname+ StringUtil.replace("();\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("清空", langArg)));
		}
		sb.append("<script type=\"text/javascript\">");
		sb.append("var windowapi = frameElement ? frameElement.api : null,W = windowapi==null?null:windowapi.opener;");
