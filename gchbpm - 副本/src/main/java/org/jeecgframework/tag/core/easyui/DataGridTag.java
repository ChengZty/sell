		...
		if (null!=operationCodes) {
			for (String MyoperationCode : operationCodes) {
				...
				systemService = ApplicationContextUtil.getContext().getBean(
							SystemService.class);
				TSOperation operation = systemService.flushEntity(TSOperation.class, MyoperationCode);
		...
		if (!StringUtils.isBlank(dictionary)&&(!popup)) {
			if(dictionary.contains(",")){
				...
				String sql = "select " + dic[1] + " as field," + dic[2]
						+ " as text from " + dic[0];
				if(dic.length>3&&StringUtil.isNotEmpty(dic[3])){
					sql += " where "+dic[3];
				}
				...
				if(redisService==null){
					redisService = ApplicationContextUtil.getContext().getBean(RedisService.class);
				}
				String val = redisService.get(dictionary);
...
//				}
	...
	public StringBuffer end() {
		...
		sb.append(StringUtil.replaceAll("loadMsg: \'{0}\',", "{0}", MutiLangUtil.getMutiLangInstance().getLang("数据加载中...")));
		...
		sb.append("$(\"#"+name+"\")."+grid+"(\"fixRownumber\");");
		...
		if(hasQueryColum(columnList)){
			...
			sb.append("<form id='"+name+"Form' onSubmit='return false'>");
					...
					if (col.isQuery()) {
						...
						if("single".equals(col.getQueryMode())){
							if(!StringUtil.isEmpty(col.getReplace())){
								...
								sb.append(StringUtil.replaceAll("<option value =\"\" >{0}</option>", "{0}", "---所有---"));
...
//									System.out.println(dic[0]+"--"+dic[1]+"--"+dic[2]+"--"+dic[3]);
...
//									}
									...
									String val = redisService.get(col.getDictionary());
									TypegroupVo groupVo = JSONHelper.fromJsonToObject(val, TypegroupVo.class);
									List<TypeVo> typeList =  groupVo.getTypeList();
									if (typeList != null) {
										for (TypeVo type : typeList) {
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			...
			sb.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\""+  name+ StringUtil.replaceAll("search()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("查询")));
			sb.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("重置")) );
			if(queryBuilder){
				sb.append("<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\"queryBuilder('"+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("高级查询")) );
			}sb.append("</span>");
		}else if("single".equals(getQueryMode())&& hasQueryColum(columnList)){//如果表单是单查询
			sb.append("<span style=\"float:right\">");
			sb.append("<input id=\""+name+"searchbox\" class=\"easyui-searchbox\"  data-options=\"searcher:"+name+ StringUtil.replaceAll("searchbox,prompt:\'{0}\',menu:\'#", "{0}", MutiLangUtil.getMutiLangInstance().getLang("请输入关键字")) +name+"mm\'\"></input>");
	...
	protected void getOptUrl(StringBuffer sb) {
		...
		for (DataGridUrl dataGridUrl : list) {
			...
			if (OptTypeDirection.Fun.equals(dataGridUrl.getType())) {
				//替换参数里面的空格和单引号
				sb.append(TagUtil.handleRecObject(dataGridUrl.getFunname()));
...
//            update-end-Author:zhangguoming  Date:20140921 for：TASK #458 列表hidden=false，才是隐藏好像有点问题
					...
					if (column.getImageSize() != null) {// 自定义显示图片
	                    ...
	                    if(tld.length==2){
	                    	...
	                    	sb.append(" return '<img width=\"" + tld[0]+ "\" height=\"" + tld[1]
	                    					+ "\" border=\"0\" src=\"'+value+'\"/>';}");
	                    }else if(tld.length==1){//指定高度
	                    	sb.append(",formatter:function(value,rec,index){");
	                    	sb.append(" return '<img  height=\"" + tld[0] + "\" border=\"0\" src=\"'+value+'\"/>';}");
	                    }
	                    tld = null;
					}else{
						sb.append(",formatter:function(value,rec,index){");
					...
					for (ColumnValue columnValue : columnValueList) {
						if (columnValue.getName().equals(column.getField())) {
							...
							for(int j = 0; j < value.length; j++){
								//替换单引号
								sb.append("if(valArray[k] == '" + value[j] + "'){ checkboxValue = checkboxValue + \'" + text[j].replace("\'", "\\\'") + "\' + ',';}");
							}
							sb.append("}");
							...
							sb.append("else{");
							for (int j = 0; j < value.length; j++) {
								testString += "if(value=='" + value[j] + "'){return \'" + text[j].replace("\'", "\\\'") + "\';}";
							}
	...
	protected void setPager(StringBuffer sb, String grid) {
		...
		if (showText) {
			sb.append("displayMsg:\'{from}-{to}" + MutiLangUtil.getMutiLangInstance().getLang("共") + " {total}" + MutiLangUtil.getMutiLangInstance().getLang("条") + "\',");
		} else {
...
//									}
		...
		if("group".equals(getQueryMode()) && hasQueryColum(columnList)){//如果表单是组合查询
			sb.append("<span style=\"float:right\">");
			sb.append("<a href=\"#\" class=\"button\" iconCls=\"icon-search\" onclick=\""+  name+ StringUtil.replaceAll("search()\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("查询")));
			sb.append("<a href=\"#\" class=\"button\" iconCls=\"icon-reload\" onclick=\"searchReset('"+name+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("重置")) );
			if(queryBuilder){
				sb.append("<a href=\"#\" class=\"button\" iconCls=\"icon-search\" onclick=\"queryBuilder('"+ StringUtil.replaceAll("')\">{0}</a>", "{0}", MutiLangUtil.getMutiLangInstance().getLang("高级查询")) );
			}sb.append("</span>");
		}else if("single".equals(getQueryMode())&& hasQueryColum(columnList)){//如果表单是单查询
			sb.append("<span style=\"float:right\">");
			sb.append("<input id=\""+name+"searchbox\" class=\"easyui-searchbox\"  data-options=\"searcher:"+name+ StringUtil.replaceAll("searchbox,prompt:\'{0}\',menu:\'#", "{0}", MutiLangUtil.getMutiLangInstance().getLang("请输入关键字")) +name+"mm\'\"></input>");
...
//		}
		...
		String val = redisService.get("rulecon");
		TypegroupVo groupVo = JSONHelper.fromJsonToObject(val, TypegroupVo.class);
		List<TypeVo> typeList =  groupVo.getTypeList();
		if (typeList != null) {
			for (int i=0;i<typeList.size();i++){
				TypeVo type = typeList.get(i);
				appendLine(sb," {'conditionId':'"+type.getTypecode()+"','conditionName':'"
						+type.getTypename()+"'}");
