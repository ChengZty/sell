...
public abstract class GenericBaseCommonDao<T, PK extends Serializable>
	...
	/**使实体变成托管状态*/
	public <T> void evictEntity(T entity) {
	 ...
	 * @return
	 */
	public String getDeleteSqlForUpdate(String tableName,String fieldName,String id,TSUser user){
		StringBuffer sb = new StringBuffer(256);
		sb.append(" update ").append(tableName).append(" set status = 'I' ,")
		.append(" update_By = '").append(user.getUserName()).append("' ,update_Name = '").append(user.getRealName())
		.append("' ,update_date = '").append(DateUtils.getTimestamp()).append("' where ").append(fieldName).append(" = '").append(id).append("'");
		return sb.toString();
	}
	
	/**通过设置update_By，update_Name，update_date的sql条件
	 * @param id
	 * @param user
	 * @return
	 */
	public String getSetUpdateConditionByIdSql(String id,TSUser user){
		StringBuffer sb = new StringBuffer(128);
		sb.append(" update_By = '").append(user.getUserName()).append("' ,update_Name = '").append(user.getRealName())
		.append("' ,update_date = '").append(DateUtils.getTimestamp()).append("' where id = '").append(id).append("'");
		return sb.toString();
	}
