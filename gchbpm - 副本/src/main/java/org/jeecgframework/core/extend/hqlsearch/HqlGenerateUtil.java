...
//		cq.eq(common.GlobalConstants.FIELD_STATUS, common.GlobalConstants.STATUS_ACTIVE);//查询状态为A的数据
...
//		--author：龙金波 ------start---date：201504022--------for：增加一个特殊sql参数处理---------------------------------- 
	 ...
	 * @param sqlstr sql条件
	 */
	public static void installHql(CriteriaQuery cq, Object searchObj,	Map<String, String[]> parameterMap,String sqlstr) {
//		cq.eq(common.GlobalConstants.FIELD_STATUS, common.GlobalConstants.STATUS_ACTIVE);//查询状态为A的数据
		installHqlJoinAlias(cq, searchObj, getRuleMap(), parameterMap, "");
//		--author：龙金波 ------start---date：20150422--------for：增加一个特殊sql参数处理---------------------------------- 
		try{
			...
			if(!Utility.isEmpty(sqlstr)){
				cq.add(Restrictions.sqlRestriction(sqlstr));
			}
		}catch(Exception e){
			e.printStackTrace();
		}
...
//	--author：陈璞 ------end---date：20150612--------for：sql组装
	 ...
	 * @param alias
	 */
	public static String getSqlCondition(Object searchObj,
			Map<String, String[]> parameterMap, String alias) {
...
//							origDescriptors[i].getPropertyType(), cq);
				...
				if (type.contains("class java.lang")
					...
					if (value != null && !value.equals("")) {
						...
						value = PageValueConvertRuleEnum.replaceValue(rule,
								value);
						String columnName = Utility.getTableColumnName(name);
						if(HqlRuleEnum.EQ.equals(rule)){//等于
							sql.append(" and ").append(columnName).append(" ='").append(value).append("'");
						}else if(HqlRuleEnum.LIKE.equals(rule)&&(!(value+"").contains("*"))&&!"class java.lang.Integer".contains(type)){//模糊
							sql.append(" and ").append(columnName).append(" like '").append(value).append("'");
						}
					} else if (parameterMap != null) {
						//update-end--Author:ll3231@126.com  Date:20151222 for：支持所有类型数据
						Object beginValue_=null , endValue_ =null;
...
//								HqlRuleEnum.LE, endValue_);
						String columnName = Utility.getTableColumnName(name);
						if(Utility.isNotEmpty(beginValue_)){
							sql.append(" and ").append(columnName).append(">='").append(beginValue_).append("'");
						}
						if(Utility.isNotEmpty(endValue_)){
							sql.append(" and ").append(columnName).append("<='").append(endValue_).append("'");
						}
					}
					// ------------update--Author:JueYue Date:2014-8-23
					// for：查询拼装的替换
					...
					if (StringUtils.isNotBlank(beginValue)) {
						if (userDefined != null) {
							sql.append(" and ").append(columnName).append(">='").append(beginValue).append("'");
						} else if (beginValue.length() == 19) {
							sql.append(" and ").append(columnName).append(">='").append(beginValue).append("'");
						} else if (beginValue.length() == 10) {
							sql.append(" and ").append(columnName).append(">='").append(beginValue + " 00:00:00").append("'");
						}
					}
					if (StringUtils.isNotBlank(endValue)) {
						if (userDefined != null) {
							...
							// 对于"yyyy-MM-dd"格式日期，因时间默认为0，故此添加" 23:23:59"并使用time解析，以方便查询日期时间数据
							sql.append(" and ").append(columnName).append("<='").append(endValue + " 23:23:59").append("'");
						}
					}
					if (isNotEmpty(value)) {
						sql.append(" and ").append(columnName).append("='").append(value).append("'");
