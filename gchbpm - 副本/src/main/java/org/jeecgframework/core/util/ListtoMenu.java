...
//		data = dataString.substring(0, dataString.toString().length()-1);
	...
	* @throws
	...
	public static String getAceMultistageTree(Map<Integer, List<TSFunction>> map,String funTp) {
		if(map==null||map.size()==0||!map.containsKey(0)){return "不具有任何权限,\n请找管理员分配权限";}
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(0);
        int curIndex = 0;
            for (TSFunction function : list) {
               ...
               /* int submenusize = function.getSubFunctionSize();
                ...
                if(!function.hasSubFunction(map)){
                	...
                	menuString.append(getACESubMenu(function,1,map,funTp));
                	menuString.append("</ul></li>");
                }
                curIndex++;
            }
	...
	private static String getACESubMenu(TSFunction parent,int level,Map<Integer, List<TSFunction>> map,String funTp){
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())){
				if(!function.hasSubFunction(map)){
					menuString.append(getLeafOfACETree(function,funTp));
				}
			}
		}
		return menuString.toString();
	}
	private static String getLeafOfACETree(TSFunction function,String funTp){
		StringBuffer menuString = new StringBuffer();
		String icon = "folder";
		if (function.getTSIcon() != null) {
			icon = TSIcon.allTSIcons.get(function.getTSIcon().getId()).getIconClas();
