...
public class SysThemesUtil {
	 ...
	 * @return
	 */
	public static SysThemesEnum getSysTheme1(HttpServletRequest request){
		String indexStyle = null;
		try {
			Cookie[] cookies = request.getCookies();
			for (Cookie cookie : cookies) {
				...
				if (cookie.getName().equalsIgnoreCase("SHARPINDEXSTYLE")) {
					indexStyle = cookie.getValue();
				}
			}
		} catch (Exception e) {
		}
		return SysThemesEnum.toEnum1(indexStyle);
	}
	/**
	 * 获取定制风格(尖刀产品)
	 * @param request
	 * @return
	 */
	public static SysThemesEnum getSysTheme2(HttpServletRequest request){
		String indexStyle = null;
		try {
			Cookie[] cookies = request.getCookies();
			for (Cookie cookie : cookies) {
		...
		return SysThemesEnum.toEnum2(indexStyle);
	}
