...
//---author:jg_xugj----end-----date:20151226--------for：修改bug 1、key.contains("${")  应改为 key.contains("#{") 2、StringUtil.isEmpty(key) 判断 不为空
	 ...
	 * @return retailerId
	 */
	public static String getRetailerType(){
		TSUser user = getSessionUserName();
		return user.getRetailerType();
	}
	
	/**获取登录人所属零售商版本
	 * @return retailerId
	 */
	public static String getRetailerEdition(){
		TSUser user = getSessionUserName();
		return user.getRetailerEdition();
	}
	
	/**获取登录人所属零售商code
	 * @return userCode
	 */
	public static String getRetailerCode(SystemService systemService){
		TSUser user = getSessionUserName();
		if(TSUser.USER_TYPE_02.equals(user.getUserType())){//零售商
		}else if(TSUser.USER_TYPE_05.equals(user.getUserType())){//员工
			user = systemService.get(TSUser.class, user.getRetailerId());
		}else{
			return null;
		}
