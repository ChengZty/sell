...
public class RoleController extends BaseController {
	...
	public ModelAndView role() {
		...
		return new ModelAndView("system/role/roleList2");
	}
	...
	public void roleGrid(TSRole role, HttpServletRequest request,
		...
		}else if(common.GlobalConstants.USER_TYPE_01.equals(user.getUserType())){//后台
...
//            systemService.executeSql("delete from t_s_role_org where role_id=?", role.getId()); // 删除 角色-机构 关系信息
            ...
            role.setStatus(common.GlobalConstants.STATUS_INACTIVE);
			userService.updateEntitie(role);
	...
	public ValidForm checkRole(TSRole role, HttpServletRequest request,
		...
		List<TSRole> roles = null;
		TSUser user = ResourceUtil.getSessionUserName();
		String hql = null;
		if(common.GlobalConstants.USER_TYPE_01.equals(user.getUserType())){
			hql = "from TSRole where retailerId is null";
			
		}else if(common.GlobalConstants.USER_TYPE_02.equals(user.getUserType())){
			hql = "from TSRole where retailerId ='"+user.getId()+"'";
		}else if(common.GlobalConstants.USER_TYPE_05.equals(user.getUserType())){
			hql = "from TSRole where retailerId ='"+user.getRetailerId()+"'";
		}
		hql +=" and rolecode = '"+roleCode+"' and status = 'A'";
		roles = systemService.findByQueryString(hql);
//		roles = systemService.findByProperty(TSRole.class,	"roleCode", roleCode);
	...
	public AjaxJson saveRole(TSRole role, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(role.getId())) {
			...
			if(StringUtil.isEmpty(role.getRetailerId())){
				role.setRetailerId(null);
			}
			...
			TSUser user = ResourceUtil.getSessionUserName();
			if(common.GlobalConstants.USER_TYPE_02.equals(user.getUserType())){//零售商
				role.setRetailerId(user.getId());
			}else if(common.GlobalConstants.USER_TYPE_05.equals(user.getUserType())){
				role.setRetailerId(user.getRetailerId());
			}
	...
	public void roleUserDatagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        */
		...
		if(common.GlobalConstants.USER_TYPE_01.equals(nowUser.getUserType())){
			String[] user_type = new String[]{"01","02"} ;
			cq.in("userType", user_type);//
		}else	if(common.GlobalConstants.USER_TYPE_02.equals(nowUser.getUserType())||common.GlobalConstants.USER_TYPE_05.equals(nowUser.getUserType())){
			cq.eq("userType", common.GlobalConstants.USER_TYPE_05);//
...
//				systemService.batchSave(roleOrgList);
	...
	@ResponseBody
	public AjaxJson doRemove(HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		try {
			String userId = request.getParameter("id");
			String roleId = request.getParameter("roleId");
			systemService.executeSql("delete from t_s_role_user where userId = ? and roleId = ?",userId,roleId);
			j.setMsg("角色移除成功成功");
		} catch (Exception e) {
			logger.error(ExceptionUtil.getExceptionMessage(e));
			j.setMsg("角色移除失败");
...
//		TSUser user = ResourceUtil.getSessionUserName();
		//角色除了后台就是零售商
		if(common.GlobalConstants.ROLE_TYPE_01.equals(role.getRoleType()) ){
			cq.eq("funFlag", "01");//后台菜单
		}else{
			cq.eq("funFlag", "02");//零售商菜单
		}
		cq.add();
//		List<TSFunction> functionList  = systemService.getListByCriteriaQuery(cq, false);
		List<TSFunction> funList  = systemService.getListByCriteriaQuery(cq, false);
		List<TSFunction> functionList  = null;
		TSUser user = ResourceUtil.getSessionUserName();
		 List<TSFunction> list = null;
		 if(common.GlobalConstants.USER_TYPE_02.equals(user.getUserType())||common.GlobalConstants.USER_TYPE_05.equals(user.getUserType())){
			 ...
			 list = systemService.findHql(hqlsb1.toString(),user.getId());
			 functionList = new ArrayList<TSFunction>();
			 for(TSFunction fun:funList){
					for(TSFunction all:list){
						if(fun.getId().equals(all.getId())){
							functionList.add(fun);
						}
					}
				}
			} else if(common.GlobalConstants.USER_TYPE_01.equals(user.getUserType())){
	...
	public ModelAndView addorupdate(TSRole role, HttpServletRequest req) {
		TSUser user = ResourceUtil.getSessionUserName();
		req.setAttribute("user_type", user.getUserType());
		if (role.getId() != null) {
			role = systemService.flushEntity(TSRole.class, role.getId());
			req.setAttribute("role", role);
			return new ModelAndView("system/role/role");
		}
		return new ModelAndView("system/role/role-add");
	}
...
//chengchuan数据规则过滤代码
    ...
    public void addUserToOrgList(TSUser user, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		...
		if(common.GlobalConstants.USER_TYPE_01.equals(nowUser.getUserType())){
			String[] user_type = new String[]{"01","02"} ;
			cq.in("userType", user_type);//
		}else	if(common.GlobalConstants.USER_TYPE_02.equals(nowUser.getUserType())){
			cq.eq("userType", common.GlobalConstants.USER_TYPE_05);//零售商员工
			cq.eq("retailerId", nowUser.getId());
    ...
    public AjaxJson doAddUserToOrg(HttpServletRequest req) {
        AjaxJson j = new AjaxJson();
        TSRole role = systemService.flushEntity(TSRole.class, req.getParameter("roleId"));
