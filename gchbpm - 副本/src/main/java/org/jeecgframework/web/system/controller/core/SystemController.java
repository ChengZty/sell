...
public class SystemController extends BaseController {
	...
	public ModelAndView typeList(HttpServletRequest request) {
		...
		TSTypegroup typegroup = systemService.flushEntity(TSTypegroup.class, typegroupid);
	...
	public void typeGroupGrid(HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid, TSTypegroup typegroup) {
        ...
        if(StringUtil.isNotEmpty(typegroupname)||StringUtil.isNotEmpty(typegroupcode)){
        	if(StringUtil.isNotEmpty(typegroupname)){
        		cq.like("typegroupname", "%"+typegroupname+"%");
        	}
        	if(StringUtil.isNotEmpty(typegroupcode)){
        		cq.like("typegroupcode", "%"+typegroupcode+"%");
        	}
        }
...
//        MutiLangUtil.setMutiLangValueForList(dataGrid.getResults(), "typegroupname");
	...
	public List<ComboTree> formTree(HttpServletRequest request,final ComboTree rootCombotree) {
		String typegroupCode = request.getParameter("typegroupCode");
		String val = redisService.get(typegroupCode);
		TypegroupVo groupVo = JSONHelper.fromJsonToObject(val, TypegroupVo.class);
		List<TypeVo> typeList =  groupVo.getTypeList();
//		TSTypegroup group = TSTypegroup.allTypeGroups.get(typegroupCode.toLowerCase());
		List<ComboTree> comboTrees = new ArrayList<ComboTree>();

		for(TypeVo tsType : typeList){
...
//		rootCombotree.setText(group.getTypegroupname());
		rootCombotree.setId(groupVo.getTypegroupcode());
		rootCombotree.setText(groupVo.getTypegroupname());
...
//    }
	...
	public AjaxJson delTypeGridTree(String id, HttpServletRequest request) {
		...
		if (id.startsWith("G")) {//分组
			TSTypegroup typegroup = systemService.flushEntity(TSTypegroup.class, id.substring(1));
			message = "数据字典分组: " + typegroup.getTypegroupname() + "被删除 成功";
			systemService.delete(typegroup);
		} else {
			TSType type = systemService.flushEntity(TSType.class, id.substring(1));
...
//		systemService.refleshTypeGroupCach();
	...
	public AjaxJson delTypeGroup(TSTypegroup typegroup, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		typegroup = systemService.flushEntity(TSTypegroup.class, typegroup.getId());
		message = "类型分组: " + typegroup.getTypegroupname() + " 被删除 成功";
        if (ListUtils.isNullOrEmpty(typegroup.getTSTypes())) {
            systemService.delete(typegroup);
            systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
            //刷新redis
            redisService.del(typegroup.getTypegroupcode());
            //刷新缓存
//            systemService.refleshTypeGroupCach();
        } else {
            message = "类型分组: " + typegroup.getTypegroupname() + " 下有类型信息，不能删除！";
        }
		j.setMsg(message);
		return j;
	}
	...
	public AjaxJson delType(TSType type, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		type = systemService.flushEntity(TSType.class, type.getId());
...
//		systemService.refleshTypesCach(type);
		systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
		//刷新redis
		String typegroupcode = this.systemService.get(TSTypegroup.class, type.getTSTypegroup().getId()).getTypegroupcode();
		String typegrp = redisService.get(typegroupcode);
		TypegroupVo groupVo = JSONHelper.fromJsonToObject(typegrp, TypegroupVo.class);
		List<TypeVo> typeList =  groupVo.getTypeList();
		...
		try {
			redisService.set(typegroupcode, MAPPER.writeValueAsString(groupVo));
		} catch (IOException e) {
			LogUtil.error("字典redis删除出错："+e.getMessage());
			e.printStackTrace();
		}
		j.setMsg(message);
		return j;
	...
	public AjaxJson saveTypeGroup(TSTypegroup typegroup, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(typegroup.getId())) {
			...
			//更改字典名称，刷新redis
			String typegrp = redisService.get(typegroup.getTypegroupcode());
			HashMap<String, Object> obj =  (HashMap<String, Object>) JSONUtils.parse(typegrp);
			obj.put("typegroupname", typegroup.getTypegroupname());
			try {
				redisService.set(typegroup.getTypegroupcode(), MAPPER.writeValueAsString(obj));
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			//新增字典，刷新redis
			TypegroupVo typegrpVo = new TypegroupVo();
			typegrpVo.setId(typegroup.getId());
			typegrpVo.setTypegroupcode(typegroup.getTypegroupcode());
			typegrpVo.setTypegroupname(typegroup.getTypegroupname());
			try {
				redisService.set(typegrpVo.getTypegroupcode(), MAPPER.writeValueAsString(typegrpVo));
			} catch (IOException e) {
				e.printStackTrace();
			}
...
//		systemService.refleshTypeGroupCach();
	...
	public AjaxJson saveType(TSType type, HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(type.getId())) {
			...
			typeVo.setTypecode(type.getTypecode());
			typeVo.setTypename(type.getTypename());
			typeVo.setTypegroupid(type.getTSTypegroup().getId());
			typeList.add(typeVo);
			try {
				redisService.set(typegroupcode, MAPPER.writeValueAsString(groupVo));
			} catch (IOException e) {
				LogUtil.error("字典redis更新出错:"+e.getMessage());
				e.printStackTrace();
			}
		}
		//刷新缓存
//		systemService.refleshTypesCach(type);
	...
	public AjaxJson delRole(TSRole role, String ids, HttpServletRequest request) {
		...
		AjaxJson j = new AjaxJson();
		role = systemService.flushEntity(TSRole.class, role.getId());
	...
	public AjaxJson delVersion(TSVersion version, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		version = systemService.flushEntity(TSVersion.class, version.getId());
	...
	public AjaxJson delDocument(TSDocument document, HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		document = systemService.flushEntity(TSDocument.class, document.getId());
	...
	public ModelAndView editFiles(TSDocument doc, ModelMap map) {
		if (StringUtil.isNotEmpty(doc.getId())) {
			doc = systemService.flushEntity(TSDocument.class, doc.getId());
            map.put("doc", doc);
		}
		return new ModelAndView("system/document/files");
	}
	...
	public AjaxJson saveFiles(HttpServletRequest request, HttpServletResponse response, TSDocument document) {
		...
		if (StringUtil.isNotEmpty(fileKey)) {
			...
			document = systemService.flushEntity(TSDocument.class, fileKey);
	 ...
	 * @return
	 */
	@RequestMapping(params = "clearIndexCache")
	@ResponseBody
	public AjaxJson clearIndexCache(HttpServletRequest request) {
		AjaxJson j = new AjaxJson();
		message = "缓存清理成功";
		String ctype = "";
		try
		{
			...
			if(cacheType.equals("app")){
				cacheType = "getHomePage*";
				ctype = "APP首页";
			}else if(cacheType.equals("active")){
				cacheType = "getActivityGoods*";
				ctype = "活动商品";
			}else if(cacheType.equals("brand")){
				cacheType = "getAllBaseBrand*";//导购端品牌列表的缓存，getBrands为顾客端的
				ctype = "品牌类目";
			}else if(cacheType.equals("brandgoods")){
				cacheType = "getGoodsByBrand*";//两端统一前缀
				ctype = "品牌商品";
			}else if(cacheType.equals("ListGoodsByType")){
				cacheType = "getGoodsCategory*";//两端统一前缀
				ctype = "商品分类";
			}else if(cacheType.equals("goodslist")){
				cacheType = "getGoodsByType*";
				ctype = "商品列表";
			}else if(cacheType.equals("user")){
				cacheType = "CUST_ID_*";
				ctype = "顾客信息";
			}else if(cacheType.equals("addr")){
				cacheType = "ADDRESS*";
				ctype = "所在区域";
			}else if(cacheType.equals("newstype")){
				cacheType = "getAllNewType*";
				ctype = "资讯分类";
			}else if(cacheType.equals("ticket")){
				cacheType = "TICKET_CODE*";
				ctype = "优惠券编码";
			}else if(cacheType.equals("SceneList")){
				cacheType = "getSceneList*";
				ctype = "场景信息";
			}
			else if(cacheType.equals("AllNews")){
				cacheType = "getAllNewsByType*";
				ctype = "所有分页资讯";
			}
			else if(cacheType.equals("AllListGate")){
				cacheType = "getAllListGate*";
				ctype = "人物列表";
			}else if(cacheType.equals("ListGate")){
				cacheType = "getListGate*";
				ctype = "顾客更换绑定导购列表";
			}else if(cacheType.equals("OnlineSpace")){
				cacheType = "listTOnlineSpace*";
				ctype = "导购在线空间";
			}else if(cacheType.equals("SpeList")){
				cacheType = "getSpeList*";
				ctype = "平台专家证书";
			}else if(cacheType.equals("Category")){
				cacheType = "getTSCategory*";
				ctype = "人物分类";
			}else if(cacheType.equals("GuideById")){
				cacheType = "getGuideById*";
				ctype = "导购信息";
			}else if(cacheType.equals("GoodsDetail")){
				cacheType = "viewGoodsDetail*";
				ctype = "商品详情";
			}
			this.redisService.batchDel(cacheType+ResourceUtil.getRetailerId()+"*");
			systemService.addLog(ctype+message, Globals.Log_Type_INSERT, Globals.Log_Leavel_INFO);
		}catch(Exception e){
			e.printStackTrace();
			message = ctype+"缓存清理失败";
			throw new BusinessException(e.getMessage());
		}
		j.setMsg(ctype+message);
		return j;
	}
	
	/**
	 * 获取七牛上传文件token
	 * c 用户获取零售商code的标识（ueditor上传的时候通过此处获取零售usercode作为七牛上传的前缀）
	 */
	@ResponseBody
	@RequestMapping(params = "getQNUptoken", produces = "application/json;charset=UTF-8")
	public Map<String, Object> getQNUptoken(String c,HttpServletRequest request,HttpServletResponse response) {
		...
		try{
			...
			if(Utility.isNotEmpty(c)){
				...
				map.put("code",  retailerCode);//七牛上传 目录(零售商userCode)
			}
//			map.put("uptoken",  org.jeecgframework.core.util.UploadFile.uptoken);
	 ...
	 * 裁剪图片，上传七牛
	 */
	@RequestMapping(params = "getCropperPage")
	public ModelAndView getCropperPage(HttpServletRequest request,HttpServletResponse response) {
		String cropperWidth = request.getParameter("cropperWidth");  //裁剪宽度
		String cropperHeight = request.getParameter("cropperHeight");  //裁剪高度
		String moduleName = request.getParameter("moduleName");  //模块名称
		
		request.setAttribute("domain", GlobalConstants.DOMAIN);//七牛上传 域名
		request.setAttribute("cropperWidth", cropperWidth);
		request.setAttribute("cropperHeight", cropperHeight);
		request.setAttribute("moduleName", moduleName);
		
		return new ModelAndView("common/upload/cropper");
	}
}
