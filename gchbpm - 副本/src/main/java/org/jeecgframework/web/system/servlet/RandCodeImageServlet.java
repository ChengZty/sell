...
public class RandCodeImageServlet extends HttpServlet {
	...
	public void doGet(final HttpServletRequest request,	final HttpServletResponse response) throws ServletException,IOException {
		...
		String loginType = request.getParameter("loginType");//1代表尖刀产品
		// 在内存中创建图象
		...
		// 设定背景颜色
		if(Utility.isEmpty(loginType)){
			graphics.setColor(Color.YELLOW); // ---1
		}else if("1".equals(loginType)){
			graphics.setColor(new Color(251,176,59)); //#FBB03B 转  251,176,59
		}
...
//			graphics.setFont(new Font("Arial Black", Font.ITALIC, 18));
			...
			}else if("1".equals(loginType)){//尖刀产品字体16px;
...
enum RandCodeImageEnum {
	 ...
	 * 混合字符串
	 */
	ALL_CHAR("23456789abcdefghijkmnpqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), // 去除小写的0,1,l,o这个4个不容易区分的字符；
	/**
	 * 字符
	 */
