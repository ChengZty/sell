...
//	}
	...
	public void flushRoleFunciton(String id, TSFunction newFunction) {
		TSFunction functionEntity = this.flushEntity(TSFunction.class, id);
		if (functionEntity.getTSIcon() == null || !StringUtil.isNotEmpty(functionEntity.getTSIcon().getId())) {
			return;
		}
		TSIcon oldIcon = this.flushEntity(TSIcon.class, functionEntity.getTSIcon().getId());
	...
	@Override
	public void deleteFunction(TSFunction function) {
		TSUser user = ResourceUtil.getSessionUserName();
		String sql = this.commonDao.getDeleteSqlForUpdate("t_s_function", "id", function.getId(), user);
		this.commonDao.updateBySqlString(sql);
		this.commonDao.updateBySqlString("delete from t_s_role_function where functionid='"	+ function.getId() + "'");
	}
}
