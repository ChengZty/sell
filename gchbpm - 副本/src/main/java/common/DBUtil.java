...
public class DBUtil {
     ...
     * @param tableName 要插入的数据库的表名
     * @return map success 是否成功（true表示没有异常），affectRowCount 影响的行数
     * @throws SQLException SQL异常
     */
    public static Map<String,Object> insertAll(String tableName, List<Map<String, Object>> datas) throws SQLException {
    	Map<String,Object> map = new HashMap<String, Object>();
        /**影响的行数**/
        int affectRowCount = 0;
        boolean success = true;//没有异常
...
//            sql.append(" )");
            ...
            String sql2 = "INSERT INTO "+tableName+" ("+columnSql+" )  VALUES ("+unknownMarkSql+")";
...
//            preparedStatement = connection.prepareStatement(sql.toString());
            ...
            for (int j = 0; j < datas.size(); j++) {
                ...
                if(n==500){
                	...
                	System.out.println(count+"共耗时" + (System.currentTimeMillis() - start)+"ms");	
                }else{
                	if(count == datas.size()){
                		long start = System.currentTimeMillis();
                		preparedStatement.executeBatch();//积攒的数据执行
                		preparedStatement.clearBatch();//积攒的清楚掉
                		System.out.println("最后一次共耗时" + (System.currentTimeMillis() - start)+"ms");	
                	}
                }
            }
...
//            affectRowCount = arr.length;
            affectRowCount = count;
            System.out.println("成功了插入了" + count + "行");
        } catch (Exception e) {
            if (connection != null) {
                connection.rollback();
            }
        ...
        map.put("affectRowCount", affectRowCount);
