...
@Scope("prototype")
@Controller
@RequestMapping("/templateNewsController")
public class TemplateNewsController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(TemplateNewsController.class);
	 ...
	 * @return
	 */
	@RequestMapping(params = "templateNewsMain")
	public ModelAndView tNewsMain(HttpServletRequest request) {
		//获取行业信息
		getTemplateTrade(request);
		return new ModelAndView("com/buss/template/templateNewsMain");
	}
	/**
	 * 零售商资讯管理列表tab 页面跳转
	 * @return
	 */
	@RequestMapping(params = "templateNewsRetailerMain")
	public ModelAndView tRetailerNewsMain(HttpServletRequest request) {
		//获取行业信息
		getTemplateTrade(request);
		return new ModelAndView("com/buss/template/templateNewsRetailerMain");
	}
	...
	public ModelAndView platformNewsList(HttpServletRequest request) {
		String hql = "FROM TTemplateTypeEntity WHERE status='A' and level='1' and platformType='1' and templateType='2' ";
		...
		return new ModelAndView("com/buss/template/templateNewsList");
	}
	/**
	 * 零售商个人资讯信息表列表 页面跳转
	 * @return
	...
	public ModelAndView retailerNewsList(HttpServletRequest request) {
		...
		return new ModelAndView("com/buss/template/retailerTemplateNewsList");
	}
	/**
	 * 全部零售商资讯信息表列表 页面跳转
	 * @return
...
//		request.setAttribute("vo", vo);
		return new ModelAndView("com/buss/template/allRetailerNewsList");
	}
	/**
	 * APP首页资讯列表
	 * @return
...
//			request.setAttribute("vo", vo);
		...
		return new ModelAndView("com/buss/template/newsListOfRet");
	}
...
//		StringBuffer sqlstr = new StringBuffer(" NOT EXISTS(select	id	from t_content_news where retailer_id = '"+retailerId+"' and content_id ='"+contentId+"' and status = 'A' and news_id = this_.id)");
		if(!StringUtil.isEmpty(news_Type)){//防止查like '%2%' 的时候把like '%12%' 的结果查出来
			...
			sqlstr.append(" and news_type not in ('1','2')");//过滤管家课堂和管家故事
		}
...
//			tNews.setUpdateDate(Utility.getCurrentTimestamp());
			...
			if (StringUtil.isNotEmpty(tradeId)) {
				saveTradeTemplate(tNews, tradeId);
			}
		}catch(Exception e){
			e.printStackTrace();
			message = "资讯信息添加失败";
	...
	//保存或修改话题的关联行业
	private void saveTradeTemplate(TNewsEntity tNews, String tradeIDs) {
		String[] tradeids = tradeIDs.split(",");
		List<TTradeTemplateEntity> tradeTemplates = systemService.findByProperty(TTradeTemplateEntity.class, "templateId", tNews.getId());
		...
		for (int i = 0; i < tradeids.length; i++) {
			...
			if(flag){
				...
				tradeTemplate.setTemplateId(tNews.getId());
...
//		Set<String> newsTypeSet = new HashSet<String>();
		try {
			...
			systemService.addLog(message, Globals.Log_Type_INSERT, Globals.Log_Leavel_INFO);
//			String newsTypes = t.getNewsType();
